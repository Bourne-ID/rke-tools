...
AZURE_META_API_VERSION="2019-11-01"
...
set_azure_config() {
  ...
  if [ "$az_vm_type" = "vmss" ]; then
    ...
    local az_vm_nic=$(az vmss nic list -g ${az_resources_group} --vmss-name ${az_vm_scale_set_name} --output json --query [0].id)
    az_vm_nic="${az_vm_nic%\"}" # trim trailing quote
    az_vm_nic="${az_vm_nic##*/}" # last element after

    if [ -z "$az_subnet_name" ] ; then
      az_subnet_name=$(az vmss nic show -g ${az_resources_group} --vmss-name ${az_vm_scale_set_name} --name ${az_vm_nic} --instance-id ${az_vm_instance_id} | jq -r .ipConfigurations[0].subnet.id | cut -d "/" -f 9)
    fi
